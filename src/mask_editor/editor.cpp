...
void clearMessageArea(const EditorMask& mask) {
    for (int i = 0; i < 15; ++i) {
...
std::string selectMaskFromList(const EditorMask& mask) {
...
RunParams inputRunParams(const EditorMask& mask) {
    clearMessageArea(mask);
    ...
    if (params.maxSteps <= 0) params.maxSteps = 3000;
    ...
    params.characteristicLength =
        inputDouble("characteristicLength", params.characteristicLength);
    ...
    while (true) {
        ...
        switch (ch) {
            ...
            case 'r': {
                ...
                std::string command =
                    "./Interactive_app --solver \"" + fullPath + "\" " +
